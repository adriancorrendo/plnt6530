...
```{r rm_comparison, warning=FALSE, message=FALSE}
# MULTCOMPARISON
...
# Plotting
We can also use mapping to create multiple plots at once.
## data preparation
```{r}
# Unnest data
data_plot <- data_corn_00
```
...
methods <- c("loess", "lm")
ses     <- c(TRUE, FALSE)
titles  <- c("LOESS model, faceted by year",
             "LM model, faceted by year")
```

## Create plots with `pmap()`
```{r}
plots <- pmap(
  list(
...
## Display plots
```{r}
# use walk to print them all at once
walk(.x=plots, .f=print)
