...
p_load(sf,leaflet, geojsonio, maps) # maps
p_load(ggplot2, ggthemes, ggrepel,
       ggspatial, ggpubr) # plots
```
...
us_plot <- 
  ...
  geom_polygon(data = us_map, aes(x = long, y = lat, group = group),
               fill = "#099999", color = "black") +
  coord_fixed(1.3) +
  labs(title = "Map of the United States",
       subtitle = "States") +
  theme_base()+
  theme(panel.background = element_rect(fill = "#bde0fe"))
...
```{r}
states_out <- c("Alaska", "District of Columbia", "Hawaii", "Puerto Rico")
# create not in function
`%notin%` <- Negate(`%in%`)

## Load shp files
usa_shp <- st_read(dsn = "shp_map/us/usa.shp") %>%
  # Remove non-contiguous and territories
  # dplyr::filter(!(NAME %in% states_out))
  dplyr::filter(NAME %notin% states_out)
...
addCircleMarkers(~lon, ~lat, popup = ~city, radius = 6, color = "blue",
                  fillOpacity = 0.9) %>%
