...
```{r}
reg_noint <- lm(yield_kgha ~ -1 + nrate_kgha, data = cornn_data)
summary(reg_fit)
summary(reg_noint)
...
This parameterization estimates a mean for each treatment level directly (useful for interpretation), while still representing the same fitted values as the intercept model.
